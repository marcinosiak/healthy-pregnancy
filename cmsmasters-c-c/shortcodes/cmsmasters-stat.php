...
if ($this->stats_atts['stats_mode'] == 'bars' && $this->stats_atts['stats_type'] == 'vertical') {
	$this->stats_atts['style_stats'] .= "\n" . '#cmsmasters_stat_wrap_' . esc_attr($unique_id) . ' .cmsmasters_stat_counter_wrap:before { ' . 
		(($color != '') ? "\n\t" . cmsmasters_color_css('color', $color) : '') . 
	"\n" . '} ' . "\n";
}

if ($this->stats_atts['stats_mode'] == 'bars' && $this->stats_atts['stats_type'] == 'horizontal') {
	$this->stats_atts['style_stats'] .= "\n" . '#cmsmasters_stat_wrap_' . esc_attr($unique_id) . ' .cmsmasters_stat_inner:before { ' . 
		(($color != '') ? "\n\t" . cmsmasters_color_css('color', $color) : '') . 
	"\n" . '} ' . "\n";
}


$out = '<div id="cmsmasters_stat_wrap_' . esc_attr($unique_id) . '" class="cmsmasters_stat_wrap' . (($this->stats_atts['stats_mode'] == 'circles' || ($this->stats_atts['stats_mode'] == 'bars' && $this->stats_atts['stats_type'] == 'vertical')) ? esc_attr($this->stats_atts['stats_count']) : '') . '">' . "\n" . 
	(($content != '' && $this->stats_atts['stats_mode'] == 'bars') ? 
	'<span class="cmsmasters_stat_counter_wrap ' . 
		(($icon != '') && ($content != '' && $this->stats_atts['stats_mode'] == 'bars' && $this->stats_atts['stats_type'] == 'vertical') ? ' ' . esc_attr($icon) : '') . 
	'">' . "\n" . 
	...
	(($this->stats_atts['stats_mode'] == 'bars' && $this->stats_atts['stats_type'] == 'vertical') ? '<div class="cmsmasters_stat_container">' . "\n" : '') . 
		...
		'>' . "\n" . 
			...
			(($icon != '') && (($content != '' && $this->stats_atts['stats_mode'] == 'bars' && $this->stats_atts['stats_type'] == 'horizontal') || ($content != '' && $this->stats_atts['stats_mode'] == 'circles')) ? ' ' . esc_attr($icon) : '') . 
			'">' . "\n" . 
